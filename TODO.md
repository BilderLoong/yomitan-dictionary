...
## Enforce example collapse in the real extension (E2E)

Source: 2026-08-06 probe of `build/Merriam Webster Unabridged.zip` in the
real Yomitan fixture (Playwright).

- [ ] Decide the collapse contract: per-sense strict ("one example shown,
  all others collapsed" per sense) vs. per-group freeze (current behavior).
  - Per-sense strict changes rendering for every entry with multiple
    `.vis` groups per sense and may diverge from MWU's own site (which
    shows one example per group, in flow position).
- [ ] Add E2E assertions to `verify_styles.ts` (real extension, real ZIP):
  - Per sense (`li[data-sc-content]` leaf): at most one inline
    `example-sentence` outside `extra-examples`.
  - Every `extra-examples` is a `details` without `open` (collapsed).
  - Summary label matches contents ("N more example(s)" ↔ N contained).
  - Clicking a summary toggles `open` (interaction works).
  - Status (2026-08-06): measured on "what" — 13 blocks, all
    `details`/collapsed, labels all correct, click expands (118 examples
    visible). BUT 5 senses show 2–5 inline examples with no collapse:
    renderer collapses per `.vis` group (call sites at
    `renderStructuredContent.ts:1011,1076,1957` call `renderExampleGroup`
    per `ex-sent-group`), and source "what" (id 464223) has senses with up
    to 4 separate `.vis` groups. The strict assertion is red until the
    renderer batches groups per sense.

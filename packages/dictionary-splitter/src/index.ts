...
program
  ...
  .requiredOption(
    ...
    "Maximum number of entries per split dictionary",
  )
  ...
  .action(
    async (
      inputPath: string,
      outputDir: string,
      options: { maxEntries: string; prefix: string },
    ) => {
      try {
        ...
        const result = await splitDictionary(
          inputPath,
          outputDir,
          maxEntries,
          options.prefix,
        ...
        result.splits.forEach((split, index) => {
          console.log(
            `  Part ${index + 1}: ${split.entryCount} entries -> ${split.zipPath}`,
          );
        });

...
async function parseDictionaryDirectory(
  directoryPath: string,
): Promise<DictionaryStructure> {
  ...
  const termBankFiles = files.filter(
  ...
  const tagBankFiles = files.filter(
    (file) => file.startsWith("tag_bank_") && file.endsWith(".json"),
  );
  ...
  const kanjiBankFiles = files.filter(
    (file) => file.startsWith("kanji_bank_") && file.endsWith(".json"),
  );
  ...
  const termMetaBankFiles = files.filter(
    (file) => file.startsWith("term_meta_bank_") && file.endsWith(".json"),
  );
...
export function groupTermsBySequence(
  terms: TermEntry[],
): Map<number, TermEntry[]> {
...
export async function parseDictionary(inputPath: string): Promise<{
  ...
  if (stats.isFile() && inputPath.endsWith(".zip")) {
    ...
    const extractionProcess = Bun.spawn([
      "unzip",
      "-q",
      inputPath,
      "-d",

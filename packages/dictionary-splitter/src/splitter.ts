...
import {
  flattenTermBanks,
  getTotalTermCount,
  groupTermsBySequence,
  parseDictionary,
} from "./parser.js";
import type { DictionaryStructure, SplitResult, TermEntry } from "./types.js";
...
export async function splitDictionary(
  ...
  if (totalTerms <= maxEntries) {
    throw new Error(
      `Dictionary only has ${totalTerms} entries, which is less than or equal to the maximum ${maxEntries}. No splitting needed.`,
    );
  }
  ...
  for (let i = 0; i < splitTermGroups.length; i++) {
    ...
    await createSplitDictionary(
      dictionary,
      terms,
      splitOutputPath,
      partNumber,
      splitTermGroups.length,
      originalDictName,
    );
...
export function splitTerms(
  ...
  const sortedSequences = Array.from(sequenceGroups.keys()).sort(
    (a, b) => a - b,
  );

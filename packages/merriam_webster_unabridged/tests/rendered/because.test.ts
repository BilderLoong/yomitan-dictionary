...
test("renders the real because usage discussion as Yomitan HTML", () => {
  ...
  const summary = extraExamples.children("summary");
  ...
  expect(
    extraExamples.children('[data-sc-content="example-sentence"]').length,
  ).toBe(3);
  ...
  expect(pointer.closest('[data-sc-content="extra-examples"]').length).toBe(0);

...
test("structures a flat usage discussion explanation", () => {
  ...
  expect(unitsOf(exampleGroups[0], "extra-examples")).toHaveLength(1);
  expect(unitsOf(exampleGroups[0], "extra-examples")[0]?.open).toBe(false);
  expect(textOf(unitsOf(exampleGroups[0], "extra-examples")[0])).toContain(
    "3 more examples",
  );
